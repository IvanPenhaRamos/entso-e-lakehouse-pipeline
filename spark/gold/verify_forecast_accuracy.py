from pyspark.sql.functions import col, lit, to_date #type:ignore

from spark.spark_session import get_spark_session

def verify_forecast_accuracy(execution_date):

    spark = get_spark_session()

    df = spark.table("nessie.gold.forecast_accuracy")

    today_df = df.where(col("date") == to_date(lit(execution_date)))
    
    rows_count = today_df.count()
    
    if rows_count == 0:
        raise ValueError(f"No data written for {execution_date}")
    
    country_count = today_df.select("country_code").distinct().count()
    
    if country_count != 6:
        raise ValueError(f"Only {country_count} countries were included")
    
    print(f"forecast_accuracy verified: {rows_count} rows for {execution_date}")